...
(function() {
  ...
  function msToTime(s) {
    ...
    if (hrs > 0) { output = hours; }
    if (hrs > 0 && mins > 0) { output = output + ", "; }
    if (mins > 0) { output = output + minutes; }
    return output;
  }
  ...
  function addZipCode(zipCode) {
    return ' Ship to: <strong id="zipcode">'+zipCode+'</strong> <a href="#" id="change-zipcode">Change Zip</a><input name="zipcode-input" id="zipcode-input" class="visually-hidden" value="'+zipCode+'"/><a href="#" id="save-zipcode" class="visually-hidden">Update</a>';
  }
  ...
  function showOrderWithinMessage() {
    ...
    var newDateTimeFormat = new Intl.DateTimeFormat('en', { timeZone: 'America/Los_Angeles', weekday: 'long', month: 'short', day: 'numeric' })
    ...
    var output = '<div id="product-order-by" style="text-align: center"><p>Get it by <strong id="receive-by">'+receiveBy+'</strong> Order within '+orderBy+'.'+addZipCode(zipCode)+'</p></div>';
    ...
    $('body').on('click', '#save-zipcode', function(e){
        ...
        $('#zipcode').html($('#zipcode-input').val()).removeClass('visually-hidden');
    });
  }
  ...
  function onDOMLoaded() {
    ...
    REBASE.theme.product_photo_gallery = new Swiper('.gallery.swiper-container', {
      effect: 'slide',
      loop: true,
      speed: 300,
      slidesPerView: 1.25,
